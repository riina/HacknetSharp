...
public class CatTests : ProgramTestBase
{
    ...
    {
        UserName("a_user").Files("text:\"/home/{UserName}/test1.txt\" \"ouf\"").Build().Shell();
    ...
    {
        Admin().Files("text+++:/prot/file \"DATA\"").Build().Shell();
    ...
    {
        Unelevated().Files("fold+++:/prot/", "text+++:/prot/file \"DATA\"").Build().Shell();
    ...
    {
        Unelevated().Files("text+++:/prot/file \"DATA\"").Build().Shell();
        Assert.That(Run("cat /prot/file").NextText(), Is.EqualTo("cat: /prot/file: Permission denied\n"));
        Assert.That(ProcessCount(), Is.EqualTo(0));
    }
}
