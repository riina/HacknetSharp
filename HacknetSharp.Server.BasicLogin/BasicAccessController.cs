...
{
    public class BasicAccessController : AccessController
    {
        private ServerDatabase? _db;

        public override async Task<bool> AuthenticateAsync(string user, string pass)
        {
            _db ??= Server.Database;
        ...
        public override async Task<bool> RegisterAsync(string user, string pass, string registrationToken)
        {
            _db ??= Server.Database;
        ...
        public override async Task<bool> ChangePasswordAsync(string user, string pass, string newPass)
        {
            _db ??= Server.Database;
            var userObj = await _db.GetAsync<string, UserModel>(user);
            if (userObj == null) return false;
        ...
        public override async Task<bool> AdminChangePasswordAsync(string user, string newPass)
        {
            _db ??= Server.Database;
            var userObj = await _db.GetAsync<string, UserModel>(user);
            if (userObj == null) return false;
        ...
        public override async Task<bool> DeregisterAsync(string user, string pass, bool purge)
        {
            _db ??= Server.Database;
            var userObj = await _db.GetAsync<string, UserModel>(user);
            if (userObj == null) return false;
